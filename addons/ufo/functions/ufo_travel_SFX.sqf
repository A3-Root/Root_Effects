...
private _blast_wave = "#particlesource" createVehicleLocal getPosATL _orb_reper;
...
private _orb_obj = "#lightpoint" createVehicleLocal getMarkerPos _ufo;
...
private _orbit_speed=0.02;

private _jumps = 4 + round (random 33);
[_orb_obj] spawn {
	params ["_orb_obj"];
...
while {_jumps > 0} do {
	...
	private _poz_rel = _orb_reper getRelPos [_radius, round (random 360)];
	_orb_reper setPos _poz_rel;
	private _sens = selectRandom [true,false];
	private _moveme = _orb_reper getRelDir _orb_obj;
	private _rad_lit = _orb_obj distance _orb_reper;
	private _rand_arc = round (random 180);
	while {_rand_arc > 0} do {
		private _poz_rel_1 =_orb_reper getRelPos [_rad_lit,_moveme];
