...
[LLSTRING(ModuleMeteor), [
    ...
    _results params ["_meteors", "_meteorFreq", "_comets", "_cometFreq", "_lethal", "_targetMode", "_targetRadius", "_owners"];

    if (_meteors) then {
        [QGVAR(startMeteors), [_pos, _meteorFreq, _lethal, _targetMode, _targetRadius, _owners]] call CBA_fnc_serverEvent;
    };
    if (_comets) then {
