...
[LLSTRING(ModuleMeteor), [
    ...
    _results params ["_meteors", "_meteorFreq", "_comets", "_cometFreq", "_lethal"];

    if (_meteors) then {
        [QGVAR(startMeteors), [_pos, _meteorFreq, _lethal]] call CBA_fnc_serverEvent;
    };
    if (_comets) then {
        [QGVAR(startComets), [_pos, _cometFreq]] call CBA_fnc_serverEvent;
    };
