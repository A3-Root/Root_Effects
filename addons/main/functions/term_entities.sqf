...
["Terminate Effects",[
	...
	// ["TOOLBOX:YESNO",["Terminate All Aurora Borealis","If true, all Aurora Borealis will be terminated."],false]
		...
		if (_isencounter) then { ufoencounter = false; publicVariable "ufoencounter"; };
		if (_isseeker) then { seekeron = false; publicVariable "seekeron"; };
		if (_iscropcircles) then { end_cross = false; publicVariable "end_cross"; };
		if (_ismeteor) then { alias_meteors = false; publicVariable "alias_meteors"; };
		if (_iscomet) then { alias_fallingstar = false; publicVariable "alias_fallingstar"; };
		if (_isvolcano) then { volcano = false; publicVariable "volcano"; };
		if (_isaaa) then { al_aaa = false; publicVariable "al_aaa"; };
		if (_isartillery) then { al_art = false; publicVariable "al_art"; };
		if (_ismissile) then { al_missile = false; publicVariable "al_missile"; };
		if (_istracer) then { al_tracer = false; publicVariable "al_tracer"; };
		if (_issearchlight) then { al_search_light = false; publicVariable "al_search_light"; };
