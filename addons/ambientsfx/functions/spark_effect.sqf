...
params ["_lamp", "_paz_emit", "_altitude"];
...
private _sparky_sun = selectRandom ["spark1", "spark3", "spark11", "spark2", "spark22", "spark5", "spark4"];
private _spark_type = selectRandom ["white", "orange"];

private _drop = 0.001 + (random 0.05);

private _scantei_spark = "#particlesource" createVehicleLocal (getPosATL _lamp);
