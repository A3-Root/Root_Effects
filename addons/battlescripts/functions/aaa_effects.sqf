...
private _li_aaa = "#lightpoint" createVehicleLocal getPosATL _object_name;
if !(_smokesOnly) then {
...
private _fum = "#particlesource" createVehicleLocal getPosATL _li_aaa;
...
private _fum_lung = "#particlesource" createVehicleLocal getPosATL _li_aaa;
...
while {al_aaa} do 
{
	private _rel_poz = [getPos _object_name, random _range_aaa, random 360] call BIS_fnc_relPos;
	private _zz = 150 + random 950;
	_li_aaa setPosATL [_rel_poz select 0, _rel_poz select 1, (getPosATL _object_name select 2) + ((selectRandom [1,-1]) * (random 50))];
	[_li_aaa] spawn 
	{
		private _li_aaa = _this select 0;
		private _flak_ground = "flak_ground";
	...
	private _flak_sound = selectRandom ["test_1", "test_2", "test_3", "bariera_1", "bariera_2", "bariera_3", "bariera_4", "bariera_5"];
