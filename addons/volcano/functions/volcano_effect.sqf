...
private _li_idle = "#lightpoint" createVehicle getPosATL _volcano_object;
...
[_li_idle] spawn {private _luminafoc = _this select 0; while {!isNull _luminafoc} do {_luminafoc setLightBrightness (59 + random(40)); _luminafoc setLightAttenuation [(1.5 + random(0.5)), (90 + random(10)), (290 + random(10)), 1, (150 + random(100)),1500]; uiSleep 0.1}; deleteVehicle _luminafoc};

private _coloana_fum = "#particlesource" createVehicleLocal getPos _volcano_object;
_coloana_fum setParticleCircle [0,[0,0,0]];
_coloana_fum setParticleRandom [7,[30,30,20],[10,10,15],0,0.5,[0,0,0,0.1],1,0];
_coloana_fum setParticleParams [["\a3\Data_f\ParticleEffects\Universal\Universal",16,7,48,1],"","Billboard",1,20,[0,0,30],[0,0,45],0,3,2,0,[50,100,100],[[0,0,0,0.5],[1,1,1,0.5],[0.5,0.5,0.5,0]],[0.5],0.5,0,"",QPATHTOF(functions\volcano_smoke_column.sqf),_volcano_object];
